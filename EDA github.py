...
# Path to data
zip_url = 'https://raw.githubusercontent.com/AureleData/exploratory-data-analysis-on-retail-database/blob/main/online_retail.zip'
...
col_list = ["Quantity","UnitPrice","Total_sales"]
for i in range(0,3):
    axes[i].boxplot(filtered_df1[col_list[i]],flierprops = dict(marker = "o", markerfacecolor = "blue"), vert = 0)
plt.show()

replace_with_threshold(filtered_df1, "Quantity")
replace_with_threshold(filtered_df1, "UnitPrice")
replace_with_threshold(filtered_df1, "Total_sales")
...
# If we look at it, we see that this client ordered picnic basket wicker 60 pieces times 60, refunded it using Manual
# and then bought 1 picnic basket wicker 60 pieces, could be a manipulation issue as at first he tried to order 60 picnic basket wicker small times 60 
# and refunded them.
